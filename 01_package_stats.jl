...
# ╔═╡ eaeb90ef-9aed-47ad-b65d-3e1d08d244a9
using PlutoUI

# ╔═╡ 29e10555-4614-4fef-84c0-ee4127826b0a
TableOfContents()

# ╔═╡ fa763a50-749a-4196-8f23-c6dc85c48c4d
md"""
...
# ╔═╡ 6c5c76e2-2dd4-436f-a153-bcce35666cd9
begin
	Pkg.Registry.update()
...
# ╔═╡ ff69d042-f176-4e10-97e2-1eae83c3fb4b
xticks = let
	date = unique(df.date)
	xticks = (
...
# ╔═╡ 627a887f-25c7-4785-9255-8250c489c155
let
...
let
	...
	for p in GENERAL_REGISTRY.pkgs
		...
		for (vr, names) in deps
			if latest_version in vr
				if "PythonCall" in names
					criterion = true
				end
				if "CondaPkg" in names
					criterion = true
				end
			end
		end
	...
	top_k_pkgs = sort(name_count, lt=(x, y) -> isless(x[2], y[2]), rev=true)[begin:10]
...
let
	...
	p = plot(
		title="Packages depends on Conda/PyCall", xticks=xticks, legend=:outerbottomright
	)
...
let
	p = plot(title="PyCall backend packages", xticks=xticks, legend=:outerbottomright)
	for pkgname in ["PyCall", "PyPlot", "SciPy", "IJulia", "SymPy", "Pandas"]
		df = pkg_downloads_by_date[Dict("package_name" => pkgname)]
		@df df plot!(p, :date, :downloads, label=pkgname, markershape=:x)
	end
	p
...
manifest_format = "2.0"
project_hash = "9fdd566df0194b792067d98713a7236b2e0e3f33"
